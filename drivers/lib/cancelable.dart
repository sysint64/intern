...
class FlatMapCancellable<R, T> implements Cancelable<R> {
  final Cancelable<T> parent;
  final Cancelable<R> Function(T value) map;

  FlatMapCancellable(this.parent, this.map);
...
extension CancelableReactiveExt<T> on Cancelable<T> {
  ...
  Cancelable<R> flatMap<R>(Cancelable<R> Function(T value) callback) {
    return FlatMapCancellable(this, callback);
  }
}
